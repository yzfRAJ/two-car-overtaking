...
#define OLED_SDA_Clr() GPIO_ResetBits(GPIOB,GPIO_Pin_4)//SDA
...
//void OLED_printf(uint8_t row, uint8_t col, const char *fmt,...);// col<3  row 
signed int OLED_printf(uint8_t x,uint8_t y,const char *pFormat, ...);
void OLED_clearare(uint8_t x1,uint8_t y1,uint8_t x2 ,uint8_t y2);
void OLED_Init(void);
